    ...
    <div class="container">
        ...
        <div class="row">
            ...
            @if ($user->isModerator())
                <div class="col-6">
                    ...
                    @can('revokeModerator', Auth::user())
                        <a class="badge badge-primary badge-pill text-danger" href="{{ route('profile.revokeModerator',[$user->id]) }}"
                            onclick="event.preventDefault();
                                          document.getElementById('revoke-form').submit();">
                             {{ __('Revoke moderator') }}
                         </a>

                         <form id="revoke-form" action="{{ route('profile.revokeModerator',[$user->id]) }}" method="POST" style="display: none;">
                             @csrf
                     </form>
                    @endcan
                </div>
            @else
                ...
                <div class="col-6">
                    <a class="badge badge-primary badge-pill text-light" href="{{ route('profile.grantModerator',[$user->id]) }}"
                                       onclick="event.preventDefault();
                                                     document.getElementById('grant-form').submit();">
                                        {{ __('Grant moderator') }}
                                    </a>

                                    <form id="grant-form" action="{{ route('profile.grantModerator',[$user->id]) }}" method="POST" style="display: none;">
                                        @csrf
                                </form>
                </div>
                @endcan
        ...
        @can('update', $user)
            <h4><a href="/password/reset">Reset password</a></h4>
        @endcan
    <h4>Last posts</h4>
